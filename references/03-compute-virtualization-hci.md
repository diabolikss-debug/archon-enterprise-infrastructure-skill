...
<!-- SOURCE: 22-COMPUTE-SERVER-ARCHITECTURE.md -->

# ARCHON KNOWLEDGE PACK

**DOMAIN:** Compute & Server Architecture  
**VERSION:** 1.0  
...
# 22 COMPUTE SERVER ARCHITECTURE

## 1. Purpose

This pack extends ARCHON with structured knowledge for **Compute & Server Architecture**. It complements the Core Runtime and existing discovery, sizing, evidence, assumption, RFP, BOM and anti-hallucination frameworks.
...
### 1. CPU generations cannot be compared core-for-core without normalization

CPU generations cannot be compared core-for-core without normalization. Use workload telemetry, representative benchmarks, sustained peak and per-core licensing constraints.

### 2. Evaluate socket count, cores/socket, frequency, cache, NUMA boundaries, memory channels, DIMM population, PCIe lanes, accelerator/HBA/NIC placement, boot design, power and cooling

Evaluate socket count, cores/socket, frequency, cache, NUMA boundaries, memory channels, DIMM population, PCIe lanes, accelerator/HBA/NIC placement, boot design, power and cooling.

### 3. Fewer hosts can reduce some licensing costs but increases per-host blast radius, memory concentration and N+1 requirements

Fewer hosts can reduce some licensing costs but increases per-host blast radius, memory concentration and N+1 requirements. Model surviving-node CPU and RAM separately.

### 4. Large VMs, databases and latency-sensitive workloads may make NUMA locality more important than aggregate cluster capacity

Large VMs, databases and latency-sensitive workloads may make NUMA locality more important than aggregate cluster capacity.

### 5. Validate exact CPU, DIMM, riser, adapter, PSU and firmware combinations with the current vendor configurator before an orderable BOM

Validate exact CPU, DIMM, riser, adapter, PSU and firmware combinations with the current vendor configurator before an orderable BOM.
...
<!-- END SOURCE: 22-COMPUTE-SERVER-ARCHITECTURE.md -->


---

<!-- SOURCE: 23-VIRTUALIZATION-ARCHITECTURE.md -->

# ARCHON KNOWLEDGE PACK

**DOMAIN:** Virtualization Architecture  
**VERSION:** 1.0  
**LAST REVIEWED:** 2026-09-20  
**VOLATILITY:** LOW-MEDIUM  
**EVIDENCE CLASS:** ARCHON engineering / product knowledge layer  
...
# 23 VIRTUALIZATION ARCHITECTURE

## 1. Purpose

This pack extends ARCHON with structured knowledge for **Virtualization Architecture**. It complements the Core Runtime and existing discovery, sizing, evidence, assumption, RFP, BOM and anti-hallucination frameworks.
...
### 1. Size virtualization from workload telemetry and service requirements, not VM count alone

Size virtualization from workload telemetry and service requirements, not VM count alone.

### 2. Evaluate cluster size, HA admission capacity, failure domains, CPU Ready/Co-Stop, memory pressure, NUMA, datastore design, network/storage paths and lifecycle operations

Evaluate cluster size, HA admission capacity, failure domains, CPU Ready/Co-Stop, memory pressure, NUMA, datastore design, network/storage paths and lifecycle operations.

### 3. Separate hypervisor HA, application HA and DR

Separate hypervisor HA, application HA and DR. A VM restart mechanism does not guarantee application RTO.

### 4. Licensing can materially change optimal host/socket/core topology and must be treated as a design constraint when applicable

Licensing can materially change optimal host/socket/core topology and must be treated as a design constraint when applicable.

### 5. Validate exact hypervisor version, HCL, storage multipathing, NIC/HBA drivers, firmware and guest/application support

Validate exact hypervisor version, HCL, storage multipathing, NIC/HBA drivers, firmware and guest/application support.
...
<!-- END SOURCE: 23-VIRTUALIZATION-ARCHITECTURE.md -->


---

<!-- SOURCE: 28-HCI-ARCHITECTURE.md -->

# ARCHON KNOWLEDGE PACK

**DOMAIN:** Hyperconverged Infrastructure Architecture  
**VERSION:** 1.0  
**LAST REVIEWED:** 2026-09-20  
**VOLATILITY:** LOW-MEDIUM  
**EVIDENCE CLASS:** ARCHON engineering / product knowledge layer  
...
This pack extends ARCHON with structured knowledge for **Hyperconverged Infrastructure Architecture**. It complements the Core Runtime and existing discovery, sizing, evidence, assumption, RFP, BOM and anti-hallucination frameworks.
...
### 1. HCI couples compute, storage and network scaling; determine which resource drives node count

HCI couples compute, storage and network scaling; determine which resource drives node count.

### 2. Evaluate failure domains, data locality, protection factor, rebuild/resync traffic, maintenance state, N+1/N+2, witness/quorum and network east-west bandwidth

Evaluate failure domains, data locality, protection factor, rebuild/resync traffic, maintenance state, N+1/N+2, witness/quorum and network east-west bandwidth.

### 3. Usable capacity must account for protection, metadata, reserve and operational free space

Usable capacity must account for protection, metadata, reserve and operational free space. Effective capacity is separate.

### 4. Node reduction can improve licensing but may worsen failure-state capacity and blast radius

Node reduction can improve licensing but may worsen failure-state capacity and blast radius.

### 5. Validate supported node types, disk groups, CPU/RAM, network, version interoperability and lifecycle rules

Validate supported node types, disk groups, CPU/RAM, network, version interoperability and lifecycle rules.
...
<!-- END SOURCE: 28-HCI-ARCHITECTURE.md -->
